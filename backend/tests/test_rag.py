...
class RAGTester:
    ...
    def _run_retrieval_tests(self, test_queries, question_id):
        ...
        # กำหนดค่า K สำหรับจำนวนผลลัพธ์ที่ต้องการค้นคืน
        K = 5
        ...
        for test_case in test_queries:
            ...
            if not self.use_api:
                result = self._test_retrieval_direct_with_metrics(test_case, question_id, K)
            else:
                result = self._test_retrieval_via_api_with_metrics(test_case, question_id, K)
    ...
    def _test_retrieval_direct_with_metrics(self, test_case, question_id, k=5):
        """
        ทดสอบการค้นคืนโดยตรงผ่าน RAG service พร้อมคำนวณ metrics เพิ่มเติม
        
        Args:
            test_case: คำถามและคำสำคัญที่ควรพบ
            question_id: รหัสคำถาม
            k: จำนวนผลลัพธ์ที่ต้องค้นหา
            
        Returns:
            dict: ผลการทดสอบพร้อม metrics
        """
        ...
        # สร้างรายการของข้อความที่ดึงมา
        retrieved_texts = [doc.page_content for doc in docs]
        retrieved_text = " ".join(retrieved_texts)
        
        # ตรวจสอบคำสำคัญที่พบแต่ล่ะเอกสาร
        # เราจะสร้าง relenance_list โดยให้ 1 ถ้าเอกสารมีคำสัญอย่างน้อย 1 คำ, 0 ถ้าไม่มี
        relevance_list = []
        found_keywords_per_doc = []
        ...
        for doc_text in retrieved_texts:
            ...
            found_keywords_per_doc.append(keywords_found)
            relevance_list.append(1 if keywords_found else 0)
        
        # คำนวณ Precision@K
        precision_at_k = sum(relevance_list) / len(relevance_list) if retrieved_text else 0
        
        # คำนวณ Recall@K
        # สมมุติว่าทุกคำสำคัญควรถูกพบ
        total_keywords = len(test_case['expected_keywords'])
        found_keywords = set()
        for keywords in found_keywords_per_doc:
            found_keywords.update(keywords)
            
        recall_at_k = len(found_keywords) / total_keywords if total_keywords > 0 else 0
        
        # คำนวณ F1-Score
        f1_score = self._calculate_f1_score(precision_at_k, recall_at_k)
        
        # คำนวณ MRR (Mean Reciprocal Rank)
        mrr = self._calculate_mrr(retrieved_texts, test_case['expected_keywords'])
        ...
        # นับคำสำคัญที่พบในผลลัพธ์รวมทั้งหมด
        total_found_keywords = 0
        for keywords in test_case['expected_keywords']:
            if keywords in retrieved_text:
                total_found_keywords += 1
                print(f"  ✅ พบคำสำคัญ: {keywords}")
            else:
                print(f"  ❌ ไม่พบคำสำคัญ: {keywords}")
        ...
        result = {
            ...
            "f1_score": f1_score,
            "mrr": mrr,
            "found_keywords": total_found_keywords,
    ...
    def _calculate_mrr(self, retrieved_texts, expected_keywords):
        """
        คำนวณ Mean Reciprocal Rank (MRR)
        
        Args:
            retrieved_texts: รายการข้อความที่ดึงมา
            expected_keywords: คำสำคัญที่ควรพบ
        
        Returns:
            float: MRR
        """
        # ค้นหาตำแหน่งของคำสำคัญใดๆ
        for i, doc_text in enumerate(retrieved_texts):
            for keyword in expected_keywords:
                if keyword in doc_text:
                    # พบคำสำคัญในเอกสารอันดบับที่ i (เริ่มจาก 0)
                    # MRR คำนวณมาจาก 1/(rank) โดย rank เริ่มจาก 1
                    return 1 / (i + 1)
        
        # ไม่พบคำสำคัญใดๆ
        return 0.0
    
    def _test_retrieval_via_api_with_metrics(self, test_case, question_id, k=5):
        """
        ทดสอบการค้นคืนผ่าน API ด้วยการประเมินคำตอบพร้อมคำนวณ metrics
        ...
        # สมมติว่าการประเมินมีบางส่วนของบริบทที่ดึงมา
        evaluation_text = result['evaluation']
        
        # ตรวจสอบคำสำคัญที่พบในการประเมิน
        found_keywords = []
        for keyword in test_case['expected_keywords']:
        ...
        # คำนวณ Precision@k (ในกรณีนี้เราไม่รู้จำนวนเอกสารจริงที่ดึงมา)
        precision_at_k = len(found_keywords) / len(test_case['expected_keywords'])
        
        # คำนวณ Recall@k (ในกรณีนี้เราไม่สามารถคำนวณได้แม่นยำเนื่องจากไม่รู้บริบทที่แท้จริง)
        recall_at_k = len(found_keywords) / len(test_case['expected_keywords'])
        
        # คำนวณ F1 Score
        f1_score = self._calculate_f1_score(precision_at_k, recall_at_k)
        
        # ไม่สามารถคำนวณ MRR ได้อย่างแม่นยำเนื่องจากไม่รู้ลำดับของเอกสาร
        mrr = 0 if not found_keywords else 1.0  # สมมติค่า
        
        # แสดงผลลัพธ์
        print(f"  Precision: {precision_at_k * 100:.2f}%")
        print(f"  Recall: {recall_at_k * 100:.2f}%")
        print(f"  F1 Score: {f1_score * 100:.2f}%")
    ...
    def _process_retrieval_results(self, results):
        ...
        Returns:
            dict: ผลสรุปการทดสอบ
        """
        # คำนวณค่าเฉลี่ยของตัวชี้วัดต่างๆ
        if not results:
        ...
        print(f"\nความแม่นยำเฉลี่ย (Avg Precision@k): {avg_precision * 100:.2f}%")
        print(f"ค่า Recall เฉลี่ย (Avg Recall@k): {avg_recall * 100:.2f}%")
        print(f"ค่า F1 Score เฉลี่ย: {avg_f1 * 100:.2f}%")
        print(f"ค่า MRR เฉลี่ย: {avg_mrr:.4f}")
    ...
    def _get_retrieval_accuracy_score(self):
        ...
        if "retrieval_accuracy" in self.test_results and self.test_results["retrieval_accuracy"].get("status") == "success":
            # นำค่า F1 Score มาใช้เป็นคะแนนหลัก เนื่องจากมีความสมดุลระหว่าง Precision และ Recall
            return self.test_results["retrieval_accuracy"].get("avg_f1_score", 0) * 100
        return 0
    ...
    def _display_summary_report(self, scores, grade):
        ...
        Args:
            ...
            grade: เกรด
        """
        # ดึงข้อมูล retrieval metrics
        retrieval_metrics = self.test_results.get("retrieval_accuracy", {})
        precision_score = retrieval_metrics.get("avg_precision", 0) * 100
        recall_score = retrieval_metrics.get("avg_recall", 0) * 100
        f1_score = retrieval_metrics.get("avg_f1_score", 0) * 100
        mrr_score = retrieval_metrics.get("avg_mrr", 0) * 100
        
        print("\nคะแนนสรุป (Summary Scores):")
        print(f"1. ความแม่นยำในการคืนค้น (Retrieval Metrics):")
        print(f"   - Precision@k: {precision_score:.2f}/100 คะแนน")
        print(f"   - Recall@k: {recall_score:.2f}/100 คะแนน")
        print(f"   - F1 Score: {f1_score:.2f}/100 คะแนน")
        print(f"   - MRR: {mrr_score:.2f}/100 คะแนน")
    ...
    def _create_radar_chart(self, scores, graphs_dir):
        ...
        # เพิ่มคะแนนจาก retrieval_metrics
        retrieval_metrics = self.test_results.get("retrieval_accuracy", {})
        precison_score = retrieval_metrics.get("avg_precision_score", 0) * 100
        recall_score = retrieval_metrics.get("avg_recall_score", 0) * 100
        f1_score = retrieval_metrics.get("avg_f1_score", 0) * 100
        mrr_score = retrieval_metrics.get("avg_mrr_score", 0) * 100
