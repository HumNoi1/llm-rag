  ...
  useEffect(() => {
    async function fetchClassInfo() {
      try {
        ...
        console.log("กำลังดึงข้อมูลรายวิชา ID:", classId);
        
        // กรณีที่รหัสวิชาไม่ใช่ตัวเลข ให้ทำการจำลองข้อมูล
        if (isNaN(classId)) {
          // จำลองข้อมูลสำหรับกรณีทดสอบ
          const mockClassInfo = {
            id: classId,
            name: classId === '4' ? 'คติดิจฯ' : 'รายวิชาทดสอบ',
            code: classId === '4' ? '0024' : 'TEST101',
            semester: '1',
            academic_year: '2568',
            students_count: 0,
            is_active: true,
            description: 'คำอธิบายรายวิชาทดสอบ',
            created_at: new Date().toISOString()
          };
          
          setClassInfo(mockClassInfo);
        } else {
          // ดึงข้อมูลรายวิชาจริงจาก Supabase
          ...
          if (error) {
            if (error.code === 'PGRST116') {
              ...
              console.log("ไม่พบข้อมูลในฐานข้อมูล จำลองข้อมูลแทน");
              data = {
                id: classId,
                name: 'คติดิจฯ',
                code: '0024',
                ...
                students_count: 0,
                is_active: true,
                description: 'วิชาในหมวดวิชาศึกษาทั่วไป',
                created_at: new Date().toISOString()
              };
            } else {
              throw error;
        ...
        // จำลองข้อมูลคำถามที่อัปโหลดแล้ว (ในอนาคตจะดึงจากฐานข้อมูล)
        setUploadedQuestions([
          { id: 'Q1', title: 'อธิบายหลักการ SOLID ในการออกแบบซอฟต์แวร์', uploadDate: '2025-04-15', status: 'evaluated' },
          { id: 'Q2', title: 'เปรียบเทียบวงจรชีวิตการพัฒนาซอฟต์แวร์แบบต่างๆ', uploadDate: '2025-04-18', status: 'pending' }
        ]);
        
        // จำลองข้อมูลคำตอบที่ประเมินแล้ว (ในอนาคตจะดึงจากฐานข้อมูล)
        setEvaluatedAnswers([
          { id: 1, questionId: 'Q1', studentName: 'นายกอ ขอไข่', score: 8, evaluatedDate: '2025-04-16' },
          { id: 2, questionId: 'Q1', studentName: 'นางสาวคา ขาว', score: 9, evaluatedDate: '2025-04-16' },
          { id: 3, questionId: 'Q1', studentName: 'นายงอ นอน', score: 7, evaluatedDate: '2025-04-17' }
        ]);
      } catch (error) {
          ...
          <div className="flex flex-wrap gap-4 mt-4">
            ...
            <div className="bg-purple-50 p-4 rounded-md flex items-center">
              ...
              <div>
                ...
                <p className="text-lg font-semibold">8.0 / 10</p>
              </div>
            </div>
          </div>
        </div>
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {evaluatedAnswers.map((answer) => (
                      <tr key={answer.id}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                          <button
                            onClick={() => handleViewEvaluation(answer.questionId)}
                            className="text-blue-600 hover:text-blue-900"
                          >
                            ดูรายละเอียด
                          </button>
